...
export default {
    ...
    methods: {
    	...
    	updateProfessionStore () {
    		if (Store.profession.name == undefined || Store.profession.name != this.$route.params.name) {
	    		this.$root.getProfessionStore(this.$route.params.name)
    		}
    	},
    	...
    	getSubmissions () {
            ...
            // if landed on a profession page
            ...
            axios.get(this.authUrl('profession-submissions'), {
            	params: {
			    	sort: this.sort,
	                page: this.page,
	                profession: this.$route.params.name
			    }
            }).then((response) => {
